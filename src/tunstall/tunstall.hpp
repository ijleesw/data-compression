...
/* SIMPLEST TUNSTALL encoding:
   1) build dictionary:
	  ...
	  256 entries (8bit) mapping
		if(length < 8bit) maps beginning of the word to length (6 bit e.g.) and code
...
//	int compress(std::ofstream &stream, unsigned char *data, int size); //return compressed size
//	void decompress(std::ofstream &stream, std::vector<unsigned char> &output); //allocate and decompress
...
int Tunstall::compress(std::ofstream &stream, unsigned char *data, int size) {
	Tunstall t;
...
void Tunstall::decompress(std::ofstream &stream, std::vector<unsigned char> &data) {
	Tunstall t;
